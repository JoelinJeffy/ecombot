...
# Flag to control whether to use Redis (set to True when Redis integration is complete)
# Currently using InMemorySessionService - Redis integration is a future enhancement
USE_REDIS = False
...
async def make_runner(agent) -> tuple[Runner, str, str]:
    ...
    """
    # Initialize PostgreSQL connection pool if not already done
    try:
        init_db_pool()
    ...
    # Create session (works for both InMemory and Redis-backed)
