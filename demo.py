...
# Day 03, Task 5 -- multi-turn validation from lab guide
TOOL_PROMPTS = [
    "Hi, my name is Priya.",       # stores customer_name
    "Where is my order ORD-001?",  # tool call, uses Priya in reply
    "What about ORD-002?",         # reuses stored name, second tool call
    "Can you track ZZ-999?",       # invalid format, handled gracefully
]
...
async def run_scenario(version: str, scenario: str) -> None:
    ...
    agent = build_support_agent(version)
    # One shared runner/session for the whole prompt list, so state
    # (customer_name, last_order_id) accumulates across turns --
    # required for the Day 03 multi-turn validation.
    runner, user_id, session_id = await make_runner(agent)
